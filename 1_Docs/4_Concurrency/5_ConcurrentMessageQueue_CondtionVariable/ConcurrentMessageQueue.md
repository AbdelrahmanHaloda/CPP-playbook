...
Once in a while, the system will - for no obvious reason - **wake up a thread**. If such a **spurious wake-up** happened with taking proper precautions, we would issue **wait** without new data being available (because the wake-up has not been caused by the **condition variable but by the system in this case**). To prevent the call to **wait** in this case, we have to modify the code slightly:
