...
⚠️ Avoid `[&]` in async or long-lived lambdas (dangerous lifetime issues)
...
## Lifetime pitfalls (very important)
...
If `x` goes out of scope → **dangling reference → undefined behavior**
...
- better constexpr lambda support

### C++20
- lambdas can be `constexpr` by default if possible
- templated lambdas improved
- can use lambdas in more compile-time contexts
...
- treating lambdas as “magic” instead of functors
...
2. What is a closure?
3. Difference between capture by value and reference?
4. What is `mutable` in lambda?
5. Why can lambdas cause lifetime bugs?
